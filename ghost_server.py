...
def dashboard():
    ...
    if request.method == 'POST':
        ...
        if action == 'send_coin':
            ...
            try:
                ...
                msg_html = f"<div class='msg err'>İşlem Hatası / Transaction Error: {str(e)}</div>"
            
            # Bakiyeyi güncelle / Update balance
            conn = db.get_connection()
            ...
            # Domain Kayıt İşlemi / Domain Registration Process
            ...
            # Dosya Yükleme İşlemi / File Upload Process
            ...
            if file.filename != '':
                ...
                msg_html = f"<div class='msg {'ok' if success else 'err'}'>{'Yükleme Başarılı / Upload Successful' if success else 'Yükleme Hatası / Upload Error'}: {response}</div>"


    conn = db.get_connection()
    # Kullanıcının varlıklarını ve işlemlerini al / Get user's assets and transactions
    ...
    # HATA DÜZELTME BURADA YAPILDI: Template içeriği, Jinja2 bloklarını düzgünce Flask'a geçirecek şekilde, f-string içinde kaçış karakteri (escape) olmadan hazırlandı.
    # Sadece doğrudan Python'dan gelen değişkenler ({...}) olarak değil, Jinja2'nin kendisinin işlemesi gereken {{ ... }} blokları olarak bırakıldı.
    # Jinja2 tag'lerini çevreleyen {{% ve %}} kaldırıldı, bu da Python'ın f-string'i olarak algılanan kısmı düzeltti.
    template_content = f"""
        {msg_html}
        
        <div class="tabs">
            <button class="tablinks" onclick="openTab(event, 'Wallet')">💳 Cüzdan & İşlemler / Wallet & Transactions</button>
            <button class="tablinks" onclick="openTab(event, 'Storage')">💾 Varlıklar & Yönetim / Assets & Management</button>
            <a href="/browse" class="tablinks" style="display:inline-block; padding: 14px 16px;">🌐 Ghost Tarayıcı / Ghost Browser</a>
        </div>
        ...
        <div id="Wallet" class="tabcontent">
            ...
            <div class="card">
                ...
                <p><strong>Bakiye / Balance:</strong> {{ session.get('balance', 0)|round(4) }} GHOST</p>
            </div>
            
            <h4>GhostCoin Gönder / Send GhostCoin</h4>
            <form method="post">
                <input type="hidden" name="action" value="send_coin">
                <input name="recipient" placeholder="Alıcı Cüzdan Adresi (Public Key) / Recipient Wallet Address" required>
                <input name="amount" type="number" step="0.0001" min="0.0001" placeholder="Miktar (GHOST) / Amount (GHOST)" required>
                <button type="submit">Gönder / Send</button>
            </form>
        ...
        <div id="Storage" class="tabcontent">
            <h3>💾 Varlık Kayıt & Yönetim / Asset Registration & Management</h3>
            
            <h4>.ghost Domain Kaydet (XML Bazlı Web Sitesi) / Register .ghost Domain (XML-Based Website)</h4>
            <form method="post">
                <input type="hidden" name="action" value="mint_domain">
                <input name="domain_name" placeholder="Örn: sitem.ghost / Ex: mysite.ghost" required>
                <textarea name="domain_data" rows="5" placeholder="XML/HTML Site İçeriği / XML/HTML Site Content" required></textarea>
                <button type="submit">Kaydet / Register</button>
            </form>
            
            <h4>Dosya Yükle (Görsel/Ses/Video) / Upload File (Image/Audio/Video)</h4>
            <form method="post" enctype="multipart/form-data">
                <input type="hidden" name="action" value="upload_file">
                <input type="file" name="file" required>
                <button type="submit">Yükle / Upload</button>
            </form>
...
# --- Kalan Rotalar / Remaining Routes ---

@app.route('/mine')
def mine():
    # Madencilik rotası / Mining route
    if not session.get('username'): 
        return redirect('/login')
    ...
    try:
        ...
        if success:
            msg = f"Madencilik Başarılı / Mining Successful! Yeni blok #{response} oluşturuldu. {BLOCK_REWARD} GHOST kazandınız. / New block #{response} created. You earned {BLOCK_REWARD} GHOST."
            # Bakiyeyi güncelle / Update balance
            conn = db.get_connection()
            user = conn.execute("SELECT balance FROM users WHERE wallet_public_key = ?", (session['pub_key'],)).fetchone()
            session['balance'] = user['balance']
            ...
            msg = f"Madencilik Başarısız / Mining Failed: {response}"
            
        return render_template_string(LAYOUT + f"""
            <div class='msg {'ok' if success else 'err'}'>{msg}</div>
            <h3>Madencilik Alanı / Mining Area</h3>
            <p>Sistem, zorluk seviyesi **{MINING_DIFFICULTY}** olan bir Proof-of-Work (İş İspatı) arıyor. / The system is searching for a Proof-of-Work with difficulty **{MINING_DIFFICULTY}**.</p>
            <p>Yeni blok kazdığınızda **{BLOCK_REWARD} GHOST** ödül kazanırsınız. / You earn a reward of **{BLOCK_REWARD} GHOST** when you mine a new block.</p>
            <a href="/dashboard">Cüzdana Geri Dön / Return to Wallet</a>
            <hr>
            <p>Son Blok / Last Block: #{ghost_chain.last_block()['block_index'] if ghost_chain.last_block() else 'N/A'}</p>
        """, ghost_chain=ghost_chain)
        
    except Exception as e:
        return f"<h1>MADENCİLİK HATASI / MINING ERROR:</h1><pre>{traceback.format_exc()}</pre>", 500

@app.route('/register', methods=['GET', 'POST'])
def register():
    # Kullanıcı kayıt rotası / User registration route
    try:
        if request.method == 'POST':
            ...
            return "Hata / Error: {msg} <a href='/register'>Geri / Back</a>"
        return render_template_string(LAYOUT + """
            <h3>Kayıt / Register</h3>
            <form method='post'><input name='username' placeholder='Kullanıcı Adı / Username'><input name='password' type='password' placeholder='Şifre / Password'><input name='name' placeholder='Ad / First Name'><input name='surname' placeholder='Soyad / Last Name'><input name='phone' placeholder='Tel / Phone'><input name='email' placeholder='Email'><button>Kaydol / Register</button></form>
            """)
    except Exception:
...
def verify():
    # Doğrulama rotası / Verification route
    if request.method == 'POST':
        if user_mgr.verify_user(session.get('temp_username', ''), request.form['code']):
            return redirect(url_for('login'))
        return "Hatalı Kod / Invalid Code"
    return render_template_string(LAYOUT + """
        <h3>Doğrulama / Verification</h3>
        <p>Terminaldeki kodu girin. / Enter the code from the terminal.</p>
        <form method='post'><input name='code' placeholder='Kod / Code'><button>Onayla / Confirm</button></form>
        """)

@app.route('/login', methods=['GET', 'POST'])
def login():
    # Kullanıcı giriş rotası / User login route
    if request.method == 'POST':
        conn = db.get_connection()
        ...
        if user:
            if user['is_verified'] == 0:
                ...
                return redirect(url_for('verify'))
            # Giriş başarılı, session bilgilerini ayarla / Login successful, set session variables
            session['username'] = user['username']
            session['pub_key'] = user['wallet_public_key']
            session['balance'] = user['balance']
            return redirect(url_for('dashboard'))
        return "Giriş Hatalı / Login Failed <a href='/login'>Tekrar Dene / Try Again</a>"
    return render_template_string(LAYOUT + """
        <h3>Giriş / Login</h3>
        <form method='post'><input name='username' placeholder='Kullanıcı Adı / Username'><input name='password' type='password' placeholder='Şifre / Password'><button>Giriş / Login</button></form>
        """)

@app.route('/upload', methods=['POST'])
def upload():
    # Yönlendirme (Asıl yükleme dashboard'da işlenir) / Redirection (Actual upload handled in dashboard)
    if not session.get('username'): return redirect('/login')
    return redirect(url_for('dashboard'))

@app.route('/clone_asset', methods=['POST'])
def clone_asset():
    # Varlık klonlama rotası / Asset cloning route
    if not session.get('username'): return redirect('/login')
